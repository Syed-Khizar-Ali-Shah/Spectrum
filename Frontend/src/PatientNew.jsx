...
function PatientNew() {
  ...
  useEffect(() => {
    ...
    if (selectedDoctor) {
      axios
        .get(`${baseUrl}/api/doctor/${selectedDoctor}/available-dates`)
        .then((response) => {
          setAvailableDates(response.data);
        });
    } else {
      // Clear available dates and times if no doctor is selected
      setAvailableDates([]);
      setSelectedDate("");
  ...
  useEffect(() => {
    ...
    if (selectedDate) {
      axios
        .get(`${baseUrl}/api/doctor/${selectedDoctor}/available-time-slots`, {
      ...
      // Clear available time slots and selected time if no date is selected
      setAvailableTimeSlots([]);
      setSelectedTime("");
    }
  ...
  const handleAppointmentSubmit = () => {
    ...
    console.log("Patient id:", searchPatient._id);

    // Make an Axios POST request to book the appointment
    // Example:
    axios
      ...
      .then((response) => {
        // Handle success
        alert(`Appointment made on ${selectedDate} at ${selectedTime}`);
      })
      .catch((error) => {
        // Handle error
        console.log(error);
    ...
    // Update the available time slots by removing the selected time
    const updatedTimeSlots = availableTimeSlots.filter(
  ...
  const [CLData, setCLData] = useState(initialFormData);

  console.log("cld", CLData);
  ...
  console.log("fgh", formData?.familyOcularHealth);

  //Occular Examination States
  const [OEForm, setOEForm] = useState({});
  console.log("oef", OEForm);
  ...
  const handleSaveOccularExamination = () => {
    let dataToSend = { ...OEForm, patientID: searchPatient._id };
    console.log("dts", dataToSend);

    axios
      .put(
        `${baseUrl}/api/newSightTest/occularExamination/${OEForm._id}`,
        dataToSend
      )
      .then((res) => {
        alert("Data Updated!");
  ...
  const handleCLEdit = () => {
    console.log("new", CLData);
    axios
      .put(`${baseUrl}/api/contactLense/${CLData._id}`, CLData)
      .then((res) => {
        alert("Contact Lense info updated.");
      })
      .catch((err) => {
        console.log(err);
        alert("Something went wrong");
  ...
  const handleCLDelete = () => {
    axios
      .delete(`${baseUrl}/api/contactLense/${CLData._id}`)
      .then((res) => {
        alert("Contact Lense info deleted.");
      })
      .catch((err) => {
        console.log(err);
        alert("Something went wrong");
                  ...
                  <div className="pn_two_rt">
                    ...
                    <div>
                      <input
                        ...
                        onChange={(e) =>
                          setpatientHasDiabetic(e.target.checked)
                        }
                      />
                      <div className="input_label" style={{ margin: "0" }}>
                        Diabetic
